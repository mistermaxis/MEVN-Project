<template>
  <div class="provider-container">
    <div name="provider-list" class="provider-list border-gray">
      <div class="provider-item" v-if="state.providers" v-for="provider in state.providers">
        <div name="provider-checkbox">
          <input class="provider" type="checkbox" :name="provider.name" :id="provider._id">
        ...
        <div>
          ...
          <button @click="e => deleteProvider(e)" class="icon-button">
            <span name="id" hidden>{{provider._id}}</span>
...
async function deleteProvider(e) {
  const id = e.currentTarget.children['id'].innerHTML;
